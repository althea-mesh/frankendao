    ...
    .service("Web3Service", function(
        ...
        // injected web3 provider (Metamask, mist, etc)
          ...
          factory.web3 =
            web3.currentProvider !== undefined
              ? new MultisigWeb3(web3.currentProvider)
              : new MultisigWeb3(web3);
          // Set accounts
          ...
          // }));
          factory.web3 = new MultisigWeb3(
            new MultisigWeb3.providers.HttpProvider(txDefault.ethereumNode)
          );
          // Check connection
          ...
          .open({
            ...
            controller: function(
              ...
              $scope.calculateFee = function() {
                $scope.txFee =
                  ($scope.gasLimit * ($scope.gasPrice * 1e9)) / 1e18;
              };
      ...
      factory.sendTransaction = function(method, params, options, cb) {
        ...
        factory.configureGas(gasConfigurationData, function(gasOptions) {
          var refinedTxOptions = Object.assign(
            {},
      ...
      factory.updateAccounts = function(cb) {
        if (!isElectron && factory.coinbase) {
          return factory.web3.eth.getAccounts(function(e, accounts) {
            if (e) {
              cb(e);
            } else {
              factory.accounts = accounts;
              ...
              if (
                ...
                accounts &&
                accounts.length &&
                accounts.indexOf(factory.coinbase) != -1
              ) {
                // same coinbase
              } else if (accounts) {
                factory.coinbase = accounts[0];
          ...
          getChainID: function(cb) {
            if (!Connection.isConnected) {
              if (txDefault.defaultChainID) {
                ...
                cb(
                  "You must set an offline Chain ID in order to sign offline transactions"
                );
              }
            } else {
              if (factory.web3) {
      ...
      factory.ledgerElectronSetup = function() {
        ...
        var web3Provider = new HookedWalletSubprovider({
          getAccounts: function(cb) {
            $http
              .get("http://localhost:" + ledgerPort + "/accounts")
          ...
          signTransaction: function(txData, cb) {
            function sendToLedger(chainID) {
              $http
                .post("http://localhost:" + ledgerPort + "/sign-transaction", {
                  tx: txData,
                  chain: chainID
                })
                .then(
        ...
        var web3Provider = new HookedWalletSubprovider({
          getAccounts: function(cb) {
            if (!factory.accounts.length) {
              TrezorConnect.ethereumGetAddress("m/44'/60'/0'/0/0", function(
                response
              ) {
                if (response.success) {
            ...
            $uibModal.open({
              ...
              controller: function($scope, $uibModalInstance) {
                ...
                $scope.ok = function() {
                  ...
                  factory.decryptLightWallet(
                    txData.from,
                    $scope.password,
                    function(response, v3Instance) {
                      if (!response) {
                        // Disable spinner
      ...
      factory.createLightWallet = function(password, ctrlCallback) {
        ...
        localStorage.setItem("pk", keyring.wallets[0].getPrivateKeyString());
        ...
        // Encrypt V3
        encryptor.encrypt(password, v3String).then(function(encryptedV3String) {
      ...
      factory.decryptLightWallet = function(address, password, callback) {
        ...
        encryptor
          .decrypt(password, encryptedV3String)
      ...
      factory.restoreLightWallet = function(password) {
        ...
        if (factory.getKeystore() && password) {
          try {
            encryptor
              .decrypt(password, factory.getKeystore())
              .then(function(decryptedV3String) {
